...
exports.render = function(patch) {
  ...
  _.forEach(patch.nodes.sort(function(n1, n2){return n1.id - n2.id}), function(node) {
    ...
    switch (node.proto) {
      case 'declare':
        console.log("This is a declare!");
        rendered += mustache.render(declareTpl, {args: node.args}) + ';\n'
        rendered += mustache.render(objUnescapedArgsTpl, {args: node.args, layout: layout, proto: node.proto}) + ';\n'
        break;
      case 'text':
        console.log("This is a text!");
        console.log(node.args);
        rendered += mustache.render(textTpl, {args: node.args, layout: layout}) + ';\n'
        break;
      default:
...
var floatAtomTpl = '#X floatatom {{{layout.x}}} {{{layout.y}}} {{{layout.width}}} {{{args.0}}} {{{args.1}}} {{{layout.labelPos}}} {{{layout.label}}} {{{args.2}}} {{{args.3}}}'
  ...
  , textTpl = '#X text {{{layout.x}}} {{{layout.y}}}{{#args}} {{{.}}}{{/args}}'
  , objTpl = '#X obj {{{layout.x}}} {{{layout.y}}} {{{proto}}}{{#args}} {{.}}{{/args}}'
  , objUnescapedArgsTpl = '#X obj {{{layout.x}}} {{{layout.y}}} {{{proto}}}{{#args}} {{{.}}}{{/args}}'
  , declareTpl = '#X declare {{#args}} {{{.}}}{{/args}}'
